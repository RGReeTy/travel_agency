...
public class EditProductCommand implements Command {
	...
	/** class for the admin to edit a product */
	...
	public String execute(HttpServletRequest request) {
		...
		try {
			...
			if (PRODUCT_RECEIVER.receiverProductUpdate(product) != null) {
				request.setAttribute("message", MessageKey.EDIT_PRODUCT_SUCCESS);
				return ConfigurationManager.getProperty("path.page.success");
			} else {
				request.setAttribute("message", MessageKey.EDIT_PRODUCT_ERROR);
				return ConfigurationManager.getProperty("path.page.error");
			}
